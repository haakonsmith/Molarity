...
class ChemicalElementVisualisation extends LitElement 
{
          ...
          <g id="ring-group">
            ${this._getConfigurationByElementSymbol(this.symbol).map((electronConfiguration, index) => svg`
              ...
              <circle  
                ...
                class="ring"
              />
              <text class="element-symbol" text-anchor="middle" alignment-baseline="central" stroke-width="2px" fill="white">${this.symbol}</text>
  ...
  _getNameByElementSymbol(symbol)
  {
    ...
    if(chemicalElements[symbol] == undefined) {
      console.error("undefined chemical symbol " + symbol);
      return '';
    }
